...
NORMALIZER_ID = "atlas:normalizer:atlas.ingestion:microsoft-windows-security-event-doc"
...
def _lineage(output_record_id: str, psr: dict, profile: dict) -> dict:
    ...
    body = {
        ...
        "mapping_rule_ids": [
            "windows-security-doc.event-identity",
            "windows-security-doc.provider-channel-context",
            "windows-security-doc.documented-versions",
            "windows-security-doc.event-xml-structure",
            "windows-security-doc.documentation-status",
        ],
    }
...
def normalize_psr(
    ...
    *,
    ...
    if mapping_profile.get("lifecycle_rule", {}).get("documentation_only") != "unset-until-provider-inventory-evidence":
        raise ValueError("documentation-only Windows normalizer must not establish canonical lifecycle")
    ...
    for psr in sorted(psr_records, key=lambda r: r["parsed_record_id"]):
        if psr.get("source_id") != mapping_profile.get("source_id"):
            raise ValueError("PSR source_id does not match pinned Windows documentation source")
        if psr.get("psr_version") != "1.0.0":
            raise ValueError("unsupported PSR version")
        if psr.get("native_type") != "windows-security-event-documentation":
        ...
        if not event_id or not EVENT_ID_RE.fullmatch(event_id):
            ...
            continue
        if event_id != native.get("event_id"):
            raise ValueError(f"Windows Event ID {event_id}: native identifier and parsed event_id differ")
            ...
            "native_identifiers": [
                {
                    ...
                    "components": {
                        ...
                        "documentation_source_version": source_version,
                        "documentation_last_updated": native.get("page_last_updated"),
                        "documented_event_versions": copy.deepcopy(native.get("documented_event_versions", [])),
                        "sample_event_xml_version": native.get("sample_event_xml_version"),
                        "task": native.get("task"),
                        "opcode": native.get("opcode"),
                        "keywords": native.get("keywords"),
                        "audit_subcategory": native.get("audit_subcategory"),
                        "minimum_os_version": native.get("minimum_os_version"),
                        "event_data_fields": copy.deepcopy(native.get("event_data_fields", [])),
                    },
                }
            ],
        }
    ...
    diagnostics.append(
        "Canonical lifecycle intentionally remains unset: Microsoft Learn documentation is distinct from provider inventory evidence."
    )
    return {
        "records": records,
        "lineage": lineage,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Normalize Windows Security event documentation PSR into canonical event candidates.")
