...
> **Active workstream:** Phase 5.10 — Public Preview Readiness — ACTIVE
> **Phase 5.10.0 baseline:** MERGED / POST-MERGE BASELINE VERIFIED
> **Public Preview readiness:** BLOCKED by explicit release-readiness gates
...
Public repository visibility does **not** imply a signed public release or GA readiness. `main` is the release authority. `FIRST PREVIEW READY` is an engineering-readiness milestone backed by merged and post-merge verified evidence; the product remains pre-preview/unreleased for public distribution.

Detailed implementation state is maintained in [Current Status](docs/current-status.md), [Project State](docs/project-state.md), the [Roadmap](docs/roadmap.md), and the [Phase 5.10 Public Preview Readiness Gate Matrix](docs/releases/phase-5.10-public-preview-readiness.md). Machine-readable readiness state is published in [`phase-5.10-public-preview-readiness.json`](docs/releases/phase-5.10-public-preview-readiness.json).

## Commercial Positioning

Cyber-defense investigation is frequently slowed by fragmented context. Analysts move between SIEMs, EDR/XDR platforms, operating-system references, ATT&CK mappings, detection repositories, incident notes, vendor documentation, and threat intelligence while trying to determine what an observed event means and which evidence supports that interpretation.

ATLAS is designed to consolidate that context into an **inspectable, provenance-first analyst workbench** without making a search index, model response, or upstream source automatically authoritative.

For SOC, DFIR, threat-hunting, and security-engineering teams, the intended value is:

- faster investigation pivots without sacrificing source traceability;
- deterministic exact and lexical retrieval before optional semantic augmentation;
- claim-level provenance for material technical assertions;
- bounded relationship and graph navigation;
- verified offline knowledge delivery for disconnected or restricted environments;
- safer pack update and rollback semantics;
- a narrow desktop attack surface with explicit host/Core trust boundaries;
- a controlled foundation for future grounded AI without making AI canonical truth.
...
- blue/purple teams that require evidence-backed investigation context;
...
Typical enterprise use cases include analyst investigation support, Windows/Sysmon research, security-knowledge consolidation, offline reference packs, provenance-aware detection/hunting context, incident-response pivots, and future grounded AI assistance over controlled local knowledge.

## Product Differentiators

ATLAS is intentionally built around trust and operational boundaries rather than feature count:

- **Provenance-first:** material technical claims remain attributable to inspectable sources.
- **Deterministic-first:** identifier and lexical retrieval work without AI.
- **Offline-first:** core investigation and verified-pack workflows do not require a default network service.
- **Fail-closed:** integrity, trust, framing, and pack-state failures do not silently downgrade to weaker behavior.
- **Contract-separated:** UI, Shared Core, content trust, canonical schema, and ecosystem ownership remain explicit.
- **Release-disciplined:** engineering readiness, public-preview readiness, and environment-specific production approval are separate states.
...
**Phase 5.5 — COMPLETE / MERGED / POST-MERGE VERIFIED / FROZEN**

Delivered capabilities include TUF content trust, secure `.atlaspack` extraction/verification, offline verification, exact artifact binding, trusted-time/highest-seen anti-rollback controls, immutable generations, atomic activation/LKG recovery, deterministic verified-pack building, production Go Shared Core, canonical/search/graph read models, durable pack trust state, and bounded local stdio IPC.

Phase 5.5 is frozen. Later desktop and release-readiness work consumes this boundary rather than redefining it.

## Windows Desktop First Preview

**Phase 5.6 — COMPLETE / MERGED / POST-MERGE VERIFIED**

The Windows-first desktop path completed executable multi-framework evaluation, mandatory hard gates G-D1 through G-D9, evidence-based host selection, First Preview UI delivery, portable packaging, and clean-Windows smoke validation. ADR-0026 records **Tauri 2.x** as the selected host.

### First Preview capabilities
...
Post-merge workflow `Phase 5.6.4 Windows First Preview Package`, run `35133827422`, completed successfully on the immutable First Preview package baseline `main@70afc6fdb9e5ce88afdb0dd4de139aa659606f1e`.

Verified controls include:

- package ZIP SHA-256 plus host/sidecar hash and size binding;
...
The First Preview artifact is intentionally an **unsigned portable ZIP**. Production Authenticode signing, installer/public distribution hardening, and binary auto-update remain release-readiness boundaries.
...
**ACTIVE — 5.10.0 BASELINE MERGED / POST-MERGE VERIFIED — PUBLIC PREVIEW BLOCKED**

Phase 5.10 converts public-release readiness from marketing language into explicit, machine-enforced evidence gates. The 5.10.0 baseline is merged and post-merge validated while preserving the immutable First Preview package baseline and frozen architecture/security contracts.
...
The initial usable domain is intentionally focused:
...
FIRST PREVIEW READY — ENGINEERING READINESS        COMPLETE
        ↓
Phase 5.10.0 readiness baseline                   MERGED / VERIFIED
        ↓
