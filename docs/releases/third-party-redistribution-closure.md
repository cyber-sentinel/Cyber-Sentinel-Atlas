...
Current desktop UI preflight: the packaged Tauri web surface contains only repository-owned `index.html`, `main.js`, and `styles.css`; no external CDN, remote font, `@font-face`, or remote asset URL is referenced. The Phase 5.10.1 CI regenerates an exact SHA-256 inventory for frontend assets and any asset-like packaging files outside `www`, rejects remote references in text-like packaged assets, and records independent deterministic evidence for the build-generated Windows `icons/icon.ico` payload including its generator-source digest, byte size and SHA-256. It also captures Windows-target (`x86_64-pc-windows-msvc`) Cargo metadata from the committed lock, requires every reachable third-party crate to expose a license expression or license file, and hashes any declared license file. This remains preflight evidence only: final built/package bytes still require release-scoped inventory and package binding; this control does not make a redistribution approval.
...
- repository-visible frontend and packaging asset inputs plus the deterministic build-generated Windows icon are now CI-inventoried; final built/package assets/fonts/icons still require release-scoped inventory and package binding;
